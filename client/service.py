...
class ChatService:
    ...
    def set_peer(self, target: str):
        """Establece el usuario con quien se está chateando."""
        self.state.current_peer = target

    async def _on_incoming(self, data: dict):
        """Procesa eventos del servidor y actualiza el AppState correspondiente."""
        cmd = data.get("payload", {}).get("command")

        # Auto-actualizar peer al aceptar una solicitud de chat
        if cmd == CommandType.ACCEPT_CHAT:
            # El peer es quien aceptó el chat (el sender del mensaje), no el target (que somos nosotros)
            peer = data.get("sender") or data.get("payload", {}).get("target")
            if peer:
                self.set_peer(peer)
                self.state.status = ConnectionStatus.IN_SESSION

        # Actualizar lista de usuarios
        if cmd == CommandType.USER_LIST:
            self.state.connected_users = data.get("payload", {}).get("users", [])

        # Actualizar estado tras registro exitoso
        if cmd == CommandType.REGISTER_OK:
            self.state.status = ConnectionStatus.CONNECTED

        # Publicar en el EventQueue para que el Frontend lo vea por el WS local
        await self.events.publish(data)

    async def connect_to_server(self, url: str, nickname: str):
        self.state.nickname = nickname
        self.state.status = ConnectionStatus.CONNECTING
        await self.network.connect(url)
        msg = Message(
            type=MessageType.COMMAND,
            sender=nickname,
            payload={"command": CommandType.REGISTER, "nickname": nickname}
        )
        await self.network.send(msg)
    ...
    async def end_chat(self):
        """Finaliza el chat con el peer actual."""
        if self.peer:
            msg = Message(
                type=MessageType.COMMAND,
                sender=self.nickname,
